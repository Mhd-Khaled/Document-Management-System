...
public partial class ManagerPages_AddEditWorkflow : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        var getDeptOfUser = from u in dc.Users // get department id of manager to show him his documents.
                            where u.UserId == int.Parse(Session["Id"].ToString())
                            select u;
        int DeptId = 0; 
        foreach (var item in getDeptOfUser)
        {
            DeptId = item.IdDept; 
        }
        HiddenField1.Value = DeptId.ToString(); // Store value in hidden field to use it in sqldatasource.
    }
